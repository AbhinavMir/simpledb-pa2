...
public class BTreeFile implements DbFile {
    ...
    public Page readPage(PageId pid) {
        ...
        try {
            ...
            if (id.pgcateg() == BTreePageId.ROOT_PTR) {
                ...
                if (bis.skip(BTreeRootPtrPage.getPageSize() + (long) (id.pageNumber() - 1) * BufferPool.getPageSize()) != BTreeRootPtrPage.getPageSize() + (long) (id.pageNumber() - 1) * BufferPool.getPageSize()) {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm, Field f) throws DbException, TransactionAbortedException {
        ...
        if (pid.pgcateg() == BTreePageId.LEAF) { // base case
            return (BTreeLeafPage) (this.getPage(tid, dirtypages, pid, perm)); // get leaf page with perm
        }

        BTreeInternalPage internalPage = (BTreeInternalPage) (this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY)); // get internal page with READ_ONLY
        Iterator<BTreeEntry> bTreeEntryIterator = internalPage.iterator(); // get iterator
        BTreeEntry bTreeEntry = null; // initialize bTreeEntry
        while (bTreeEntryIterator != null && bTreeEntryIterator.hasNext()) { // iterate through iterator and Check if null
            bTreeEntry = bTreeEntryIterator.next(); // get next bTreeEntry
            if (f == null || f.compare(Op.LESS_THAN_OR_EQ, bTreeEntry.getKey())) { // if f is null or f is less than or equal to bTreeEntry.getKey()
                return this.findLeafPage(tid, dirtypages, bTreeEntry.getLeftChild(), Permissions.READ_ONLY, f); // recurse on left child
            }
        }

        //Probably should handle possible null pointer exception, but im lazy
        return this.findLeafPage(tid, dirtypages, bTreeEntry.getRightChild(), Permissions.READ_ONLY, f); // recurse on right child
    }
     ...
     */
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) throws DbException, IOException, TransactionAbortedException {

        BTreeLeafPage rightLeafPage = (BTreeLeafPage) (this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF)); // get empty right leaf page
        int numTuple = page.getNumTuples(); // get number of tuples in page
        int splitIndex = numTuple / 2; // get split index
        Iterator<Tuple> tupleIterator = page.reverseIterator(); // get iterator
        ...
        while (count < splitIndex && tupleIterator.hasNext()){ // Check if iterator has next and split point hasn't been reached
            currentTuple = tupleIterator.next(); //Get Rightmost tuple
            page.deleteTuple(currentTuple); //Delete Rightmost tuple
            rightLeafPage.insertTuple(currentTuple); //Insert old Rightmost tuple into new RightLeafPage
            count++; //Increase number of tuples moved
        }

        Field moveKey = currentTuple.getField(this.keyField); //Get middlemost tuples key
        BTreeInternalPage parentPage = (BTreeInternalPage) this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), moveKey); //Get Parent Node with empty slots of current leaf Page
        parentPage.insertEntry(new BTreeEntry(moveKey, page.getId(), rightLeafPage.getId())); //Copy up key to Parent Page
        rightLeafPage.setParentId(parentPage.getId()); //Assign Parent page to rightLeafPAge
        if (page.getRightSiblingId() != null) { //If the current leaf node already has a Right Sibling insert new RightPage between them
            BTreeLeafPage oldRightSibling = (BTreeLeafPage) this.getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE); //Get Old RightSibling w/RW Perms
            //Set New Sibling Ids
            oldRightSibling.setLeftSiblingId(rightLeafPage.getId());
            rightLeafPage.setRightSiblingId(oldRightSibling.getId());
        }
        //Set New Sibling Ids for split pages
        page.setRightSiblingId(rightLeafPage.getId());
        rightLeafPage.setLeftSiblingId(page.getId());

        //Return current page if copied up key is less than or equal to current pages key, else return new Right Leaf Page
        if (field.compare(Op.LESS_THAN_OR_EQ, moveKey)) return page;
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page, Field field) throws DbException, IOException, TransactionAbortedException {
        BTreeInternalPage rightInternalPage = (BTreeInternalPage) (this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL)); // get empty right internal page
        int numEntries = page.getNumEntries(); // get number of entries in page
        int splitIndex = numEntries / 2; // get split index
        Iterator<BTreeEntry> bTreeEntryIterator = page.reverseIterator(); // get iterator

        if (numEntries % 2 == 0) { // if numEntries is even
            splitIndex--; // decrement splitIndex
        }

        int count = 0; // initialize count

        while (bTreeEntryIterator.hasNext() && count < splitIndex) { // iterate through iterator
            BTreeEntry bTreeEntry = bTreeEntryIterator.next(); // get next bTreeEntry
            page.deleteKeyAndRightChild(bTreeEntry); // delete bTreeEntry from page
            rightInternalPage.insertEntry(bTreeEntry); // insert bTreeEntry into rightInternalPage
            count++; // increment count
        }

        BTreeEntry nextBTreeEntry = bTreeEntryIterator.next(); // get next bTreeEntry
        Field nextKey = nextBTreeEntry.getKey(); // get key of nextBTreeEntry
        page.deleteKeyAndRightChild(nextBTreeEntry); // delete nextBTreeEntry from page
        BTreeInternalPage parentInternalPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), nextKey); // get parentInternalPage
        nextBTreeEntry.setLeftChild(page.getId()); // set left child of nextBTreeEntry to page
        nextBTreeEntry.setRightChild(rightInternalPage.getId()); // set right child of nextBTreeEntry to rightInternalPage
        parentInternalPage.insertEntry(nextBTreeEntry); // insert nextBTreeEntry into parentInternalPage
        updateParentPointers(tid, dirtypages, rightInternalPage); // update parent pointers
        if (field.compare(Predicate.Op.LESS_THAN_OR_EQ, nextKey)) { // if field is less than or equal to nextKey
            return page; // return page
        } else { // else
            return rightInternalPage; // return rightInternalPage
        }
    }
     ...
     */
    private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId parentId, Field field) throws DbException, IOException, TransactionAbortedException {
     ...
     * @throws TransactionAbortedException
     */
    private void updateParentPointer(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, BTreePageId child) throws DbException, IOException, TransactionAbortedException {
     ...
     */
    private void updateParentPointers(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page) throws DbException, IOException, TransactionAbortedException {
     ...
     * @throws TransactionAbortedException
     */
    Page getPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm) throws DbException, TransactionAbortedException {
     ...
     */
    private void handleMinOccupancyPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePage page) throws DbException, IOException, TransactionAbortedException {
     ...
     */
    private void handleMinOccupancyLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, BTreeInternalPage parent, BTreeEntry leftEntry, BTreeEntry rightEntry) throws DbException, IOException, TransactionAbortedException {
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling, BTreeInternalPage parent, BTreeEntry entry, boolean isRightSibling) throws DbException {
        ...
        int moveNum = totalTupleNum / 2 - page.getNumTuples(); // ceiling
        int moveIndex = 0;
        Iterator<Tuple> ite = sibling.reverseIterator();
        if (isRightSibling) ite = sibling.iterator();
        Tuple currentTuple = null;
        while (moveIndex < moveNum && ite.hasNext()) {
     ...
     */
    private void handleMinOccupancyInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page, BTreeInternalPage parent, BTreeEntry leftEntry, BTreeEntry rightEntry) throws DbException, IOException, TransactionAbortedException {
        BTreePageId leftSiblingId = null;
        BTreePageId rightSiblingId = null;
        if (leftEntry != null)
            leftSiblingId = leftEntry.getLeftChild();
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page, BTreeInternalPage leftSibling, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
        // some code goes here
        int totalEntryNum = page.getNumEntries() + leftSibling.getNumEntries();
        int moveNumCeil = totalEntryNum / 2 - page.getNumEntries(); // ceiling
        int index = 0;
        Iterator<BTreeEntry> ite = leftSibling.reverseIterator();
        //Changed to use new Iterators rather than previous declared ones (ie: ite) because calling next() was messing up the ordering
        BTreePageId leftChildId = leftSibling.reverseIterator().next().getRightChild();
        BTreePageId rightChildId = page.iterator().next().getLeftChild(); // Doesn't make sense but test cases fail pls keep in mind and get clarification
                                                                            //Use A Iterator not a reverse Iterator to get the right Id
        page.insertEntry(new BTreeEntry(parentEntry.getKey(), leftChildId, rightChildId));
        BTreeEntry currentEntry = null;
        //Minus 1 because the last delete requires extra stuff
        while (index < moveNumCeil - 1 && ite.hasNext()) {
            currentEntry = ite.next();
            leftSibling.deleteKeyAndRightChild(currentEntry);
            page.insertEntry(currentEntry);
            index++;
        }
        //Handle funky final delete outside of loop (You dont insert currentEntry Back into page)
        currentEntry = ite.next();
        leftSibling.deleteKeyAndRightChild(currentEntry);
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage page, BTreeInternalPage rightSibling, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
        ...
        int totalEntryNum = page.getNumEntries() + rightSibling.getNumEntries();
        int moveNumCeil = totalEntryNum / 2 - page.getNumEntries(); // ceiling
        int index = 0;
        ...
        //Loop through and Move all but one entry (making sure to delete first then push up)
        BTreeEntry currentEntry = null;
        while (index < moveNumCeil - 1 && ite.hasNext()) {
            currentEntry = ite.next();
        ...
        //Last delete handled outside of loop (You dont push up an entry here)
        currentEntry = ite.next();
        rightSibling.deleteKeyAndLeftChild(currentEntry);
        parentEntry.setKey(currentEntry.getKey());
        parent.updateEntry(parentEntry);
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage leftPage, BTreeLeafPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
        ...
        // OK pls read the comments above

        //Loop and move all tuples from right to left
        ...
        //If Right Page isn't the rightmost page fix pointers
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage leftPage, BTreeInternalPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
        //Fixed to get correct Child Ids
        ...
        //Previous version wasn't deleting from the Right Page
        while (ite.hasNext()) {
            currentEntry = ite.next();
     ...
     */
    private void deleteParentEntry(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePage leftPage, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
     ...
     * @throws TransactionAbortedException
     */
    protected int getEmptyPageNo(TransactionId tid, HashMap<PageId, Page> dirtypages) throws DbException, IOException, TransactionAbortedException {
     ...
     */
    private Page getEmptyPage(TransactionId tid, HashMap<PageId, Page> dirtypages, int pgcateg) throws DbException, IOException, TransactionAbortedException {
        // create the new page
     ...
     */
    protected void setEmptyPage(TransactionId tid, HashMap<PageId, Page> dirtypages, int emptyPageNo) throws DbException, IOException, TransactionAbortedException {
